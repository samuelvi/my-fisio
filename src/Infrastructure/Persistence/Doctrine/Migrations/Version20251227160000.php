...
final class Version20251227160000 extends AbstractMigration
{
    ...
    {
        return 'Creates initial database schema with all tables';
    }

    public function up(Schema $schema): void
    {
        // Note: PostgreSQL extensions (unaccent, pg_trgm, fuzzystrmatch) are installed
        // via docker/postgres/01-init-extensions.sql when the database is created.
        // This keeps infrastructure separate from application schema migrations.
        ...
        // Add normalized search columns for fast case-insensitive and accent-insensitive searches
        $this->addSql('ALTER TABLE patients ADD COLUMN full_name_normalized TEXT GENERATED ALWAYS AS (unaccent(LOWER(full_name))) STORED');
        $this->addSql('ALTER TABLE invoices ADD COLUMN full_name_normalized TEXT GENERATED ALWAYS AS (unaccent(LOWER(full_name))) STORED');

        // Create indexes on normalized columns for fast searching
        $this->addSql('CREATE INDEX idx_patients_full_name_normalized ON patients (full_name_normalized)');
        $this->addSql('CREATE INDEX idx_invoices_full_name_normalized ON invoices (full_name_normalized)');
    }

    public function down(Schema $schema): void
    {
        // Drop indexes on normalized columns
        $this->addSql('DROP INDEX IF EXISTS idx_patients_full_name_normalized');
        $this->addSql('DROP INDEX IF EXISTS idx_invoices_full_name_normalized');
        ...
        // Drop tables (this will also drop the generated columns)
        ...
        // Note: We don't drop PostgreSQL extensions as they're infrastructure-level
        // and managed separately via docker/postgres/01-init-extensions.sql
    }
}
